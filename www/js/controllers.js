        ...
        if (!$scope.all_loaded) {
          ...
          $http.get($scope.api_url+$scope.page_no).success(function(response) {
            ...
            for (var j=0; j<response.length; j++) {
              ...
              $scope.getPostImgUrl($scope.postslist.length-1);
            }
            $scope.loaded = true;
            $scope.moreLoaded = true;
            $scope.$broadcast('scroll.infiniteScrollComplete');
...
//      $scope.loadPostsList();
      ...
      /** get Post Image URL **/
      $scope.getPostImgUrl = function(idx) {
        $scope.postslist[idx].img_loaded = false;
        $http.get(SERVER_INFO.API_PREFIX+'/media?parent='+$scope.postslist[idx].id).success(function(response) {
          $scope.postslist[idx].img = response[0].source_url;
          $scope.postslist[idx].img_loaded = true;
          window.localStorage['postlist'] = JSON.stringify($scope.postslist);
          console.log("Load URL("+$scope.postslist[idx].id+"):"+$scope.postslist[idx].img);
        }).error(function(error) {
        ...
        if (!$scope.all_loaded) {
          ...
          $http.get($scope.api_url+$scope.page_no).success(function(response) {
            ...
            for (var j=0; j<response.length; j++) {
              ...
              $scope.getPostImgUrl($scope.postslistbysearch.length-1);
            }
            $scope.loaded = true;
            $scope.moreLoaded = true;
            $scope.$broadcast('scroll.infiniteScrollComplete');
...
//      $scope.searchPostsList();
      ...
      /** get Post Image URL **/
      $scope.getPostImgUrl = function(idx) {
        $scope.postslistbysearch[idx].img_loaded = false;
        $http.get(SERVER_INFO.API_PREFIX+'/media?parent='+$scope.postslistbysearch[idx].id).success(function(response) {
          $scope.postslistbysearch[idx].img = response[0].source_url;
          $scope.postslistbysearch[idx].img_loaded = true;
          window.localStorage['postlist'] = JSON.stringify($scope.postslistbysearch);
          console.log("Load URL("+$scope.postslistbysearch[idx].id+"):"+$scope.postslistbysearch[idx].img);
        }).error(function(error) {
          console.log("Load Image URL error:"+error);
        });
      };
        ...
        if (!$scope.all_loaded) {
          ...
          $http.get($scope.api_url+'&page='+$scope.page_no).success(function(response) {
            ...
            for (var j=0; j<response.length; j++) {
              ...
              $scope.getPostImgUrl($scope.postslistbycat.length-1);
            }
            $scope.loaded = true;
            $scope.moreLoaded = true;
            $scope.$broadcast('scroll.infiniteScrollComplete');
...
//      $scope.loadPostsListByCategory();
      ...
      /** get Post Image URL **/
      $scope.getPostImgUrl = function(idx) {
        $scope.postslistbycat[idx].img_loaded = false;
        $http.get(SERVER_INFO.API_PREFIX+'/media?parent='+$scope.postslistbycat[idx].id).success(function(response) {
          $scope.postslistbycat[idx].img = response[0].source_url;
          $scope.postslistbycat[idx].img_loaded = true;
          window.localStorage['postlist'] = JSON.stringify($scope.postslistbycat);
          console.log("Load URL("+$scope.postslistbycat[idx].id+"):"+$scope.postslistbycat[idx].img);
        }).error(function(error) {
          console.log("Load Image URL error:"+error);
        });
      };
...
/**********************************   Post Controller    *************************************/
    ...
    .controller('PostCtrl', function($scope, $http, SERVER_INFO, $stateParams, $ionicPopup, $ionicLoading, $ionicPlatform,
      ...
      /** get Post Image URL **/
      $scope.getPostImgUrl = function() {
        $scope.postInfo.img_loaded = false;
        $http.get(SERVER_INFO.API_PREFIX+'/media?parent='+$scope.postInfo.id).success(function(response) {
          $scope.postInfo.img = response[0].source_url;
          $scope.postInfo.img_loaded = true;
          console.log("Load URL("+$scope.postInfo.id+"):"+$scope.postInfo.img);
        }).error(function(error) {
          console.log("Load Image URL error:"+error);
        });
      };
...
//                $cordovaToast.showLongCenter('Imagem salva in '+targetPath);
      ...
      $scope.downloadImg = function(imgUrl, postId) {
        document.addEventListener('deviceready', function () {
          ...
          var image_url = imgUrl.substring(0, imgUrl.length-4)+"-640x527.jpg";
          var image = new Image();
          image.onload = function () {
            var canvas = document.createElement('canvas');
            canvas.width = this.naturalWidth; // or 'width' if you want a special/scaled size
...
//                          $scope.imgFile = entry.toURL();
      ...
      $scope.shareInstagram = function(message, image, link) {
        document.addEventListener("deviceready", function () {
          var image_url = image.substring(0, image.length-4)+"-640x527.jpg";
